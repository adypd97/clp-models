...
def show(x, y, title='Avg Overall Heat Transfer Coeff of Exchanger vs DTC', xlabel='DTC', ylabel='Avg U', saveas='UvsDTC.png'):
    ...
    plt.grid(True)
    plt.plot(x, y)
    plt.legend(['x axis: Flow rate [1, 75] LPH, Hot Liquid Flow rate : 75 LPH'])
    plt.title(title)
    plt.savefig('./results/' + saveas)
    plt.show()

def run(T, T_h_i, T_c_i, T_h_o, T_c_o, dtc, m_c, flag=1):
...
def main(DTC, m_c):
    ...
    show(m_c, avg_U, title='Avg Overall Heat Transfer Coeff vs Cold Liquid Flow Rate (DTC=60C)', xlabel='m_c', saveas='Uvsm_c.png')
    show(m_c, avg_e, title='Avg Effectiveness of Exchanger vs Cold Liquid Flow Rate (DTC=60C)', xlabel='m_c', ylabel='Avg e', saveas='evsm_c.png')
...
if __name__ == "__main__":
    ...
    DTC = 60
    m_h = 75
    #m_h = [ i for i in range(100, 49, -1) ] #75   # flow rate for hot water  (kg/s) (check TODO)
